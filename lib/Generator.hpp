...
/*
    Observables
    obs_order_parameter[t] : Order parameter at time t
    obs_second_maximum[t] : Second maximum cluster at time t
...
struct Generator {
   ...
   protected:
    ...
    std::vector<std::pair<unsigned, unsigned>> obs_inter_event_time;
...
Generator::Generator(const Name& t_name,
                     const int& t_random_engine_seed) : name(t_name),
                                                        ...
                                                        enesmble_size(t_name.ensemble_size),
...
void Generator::initialize() {
    ...
    */
    ...
    if (check_list.at("order_parameter")) {
        obs_order_parameter.assign(network_size, 0.0);
    }
    if (check_list.at("second_maximum")) {
        obs_second_maximum.assign(network_size, 0.0);
    }
    ...
    if (check_list.at("inter_event_time")) {
        obs_inter_event_time.assign(network_size, std::pair<unsigned, unsigned>{0, 0});
    }

    //* Accumulated Observable
    if (check_list.at("max_delta_order_parameter")) {
        obs_max_delta_order_parameter.reserve(enesmble_size);
    }
}
...
void Generator::run() {
    ...
    */
    for (unsigned ensemble = 0; ensemble < enesmble_size; ++ensemble) {
        _run();
    }
}
...
void Generator::save() const {
    ...
    if (check_list.at("order_parameter")) {
        ...
        const std::vector<double> order_parameter = obs_order_parameter / (double)enesmble_size;
    ...
    if (check_list.at("second_maximum")) {
        ...
        CSV::generateDirectory(directory);
        const std::vector<double> second_maximum = obs_second_maximum / (double)enesmble_size;
        CSV::write(directory + name.get_NGE(), second_maximum, precision);
    }
    //! Mean Cluster Size
    if (check_list.at("mean_cluster_size")) {
        const std::string directory = data_dir + "mean_cluster_size/";
        CSV::generateDirectory(directory);
        const std::vector<double> mean_cluster_size = obs_mean_cluster_size / (double)enesmble_size;
    ...
    if (check_list.at("second_moment")) {
        ...
        CSV::generateDirectory(directory);
        const std::vector<double> second_moment = obs_second_moment / (double)enesmble_size;
...
void Generator::_run() {
    ...
    {
        if (check_list.at("order_parameter")) {
            obs_order_parameter[time] += 1.0 / network_size;
        }
        if (check_list.at("second_maximum")) {
            obs_second_maximum[time] += 1.0 / network_size;
        }
        if (check_list.at("mean_cluster_size")) {
            obs_mean_cluster_size[time] += 1.0 / network_size;
        }
        if (check_list.at("second_moment")) {
            obs_second_moment[time] += 1.0 / pow(network_size, 2.0);
        }
    }
    ...
    while (network.maximum_cluster_size < network_size) {
        ...
        if (newSize <= upper_bound) {
            ...
            const double second_maximum = network.get_second_maximum_cluster_size() / (double)network_size;
